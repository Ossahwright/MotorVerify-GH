...
export const VerificationResult: React.FC<VerificationResultProps> = ({
  ...
  // Reusable Audit & Provider Trace component (Always open, excluded from printed certificate)
  const renderAuditTrace = () => (
    <div className="mt-6 pt-4 border-t border-slate-200 text-left no-print">
      ...
      <div className="text-xs font-mono bg-slate-50 p-3.5 rounded-lg border border-slate-200 text-slate-600 space-y-1.5">
        {verificationId && (
          <div>
            <span className="font-semibold text-slate-700">Firestore ID:</span> {verificationId}
          </div>
        )}
        <div>
  ...
  if (multipleRecords && records && records.length > 0) {
    return (
        ...
        <div className="p-6 sm:p-8">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-6 border-t border-slate-100 no-print">
            ...
            <button
              type="button"
              onClick={handlePrint}
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-800 font-semibold text-sm px-5 py-2.5 rounded-lg border border-slate-200 transition-colors cursor-pointer focus:ring-2 focus:ring-slate-400 outline-none"
            >
